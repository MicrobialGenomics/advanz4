...
variables_selected <- read.csv(here::here("Metadata","MetadataManagement","Var_names_excel.csv"), header=T) %>%
...
myClinicalData<-
  # here::here("Metadata","MetadataManagement","Clinical_Table.csv") %>%
...
vars=c("CD4 value","CD8 value", "4+38+DR+","8+38+DR+", "PCR", "IL-6", "TNF-a", "sCD14","Viral Load HIV [lab]", "CMV Viral Load")
...
ALso, weight in 3 patients: C007 (w48), D009 (w96), E008 (w0) and D013(w96) was wrongly recorded, as the last decimal was not included. We will rectify that and recalculate the BMI. Patient B006 failed to record its weight at baseline
...
Some variables are coded in a way that makes analysis difficult. We will recode them to remove duplicities and sistematize it.
...
This section will be to document any errors or problems we may find during downstream analyses.

### FIx BMI
BMI was problematic and not properly calculated in some patients. Also, in some patients the fractional comma was missing, leading to outrageous weights and BMIs.

```{r fix_BMIs, warning=F, message=F}
metadata_final <- 
  metadata_final %>% 
  mutate(weight = case_when(record_id == "C007" & time_point == 48 ~ 69.7,
                            record_id == "D009" & time_point == 96 ~ 61.9,
                            record_id == "E008" & time_point == 0 ~ 39.1,
                            record_id == "D013" & time_point == 96 ~ 67,
                            TRUE ~ as.numeric(weight)),
