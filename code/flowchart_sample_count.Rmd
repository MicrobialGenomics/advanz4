...
mITT %>%
  dplyr::filter(mITT == "Yes") %>% 
  dplyr::select(PatCode, Reason, mITT)

metmITT <- metadata %>%
  select(record_id, group) %>%
  dplyr::filter(record_id %in% mITT$PatCode) %>%
  unique() %>%
  pull(record_id) 
...
metadata %>%
  filter(record_id %in% metmITT) %>%
  filter(record_id %in% mITT)

patient_data %>%
  filter(!Reason == "No\r\nPremature\r\nFinalisation") %>%
  filter(Treat == "ABC+3TC+DGT") %>% 
