...
basal_mre@metadata@categorical_vals <- tibble(CategoricalVariable = "cmv_bl_c", PaletteName = "Accent")
add_num(basal_mre, c("HIV_VL","CMV_VL"), palette = c("Set1","Set1"))
basal_mre@taxa@metaphlan@phyloseq@sam_data <- phyloseq::sample_data(bl_df)
basal_nmds <- metar::metaphlan_nmds(mre = basal_mre, tax_level = "Species", save_files = F)

basal_nmds@taxa@metaphlan@nmds$Species$top_50$categorical$cmv_bl_c
