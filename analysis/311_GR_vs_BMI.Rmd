  ...
  purrr::map_dfr(function(comb){
    ...
    delta_bmi_df %>% 
      dplyr::filter(!is.na(richness),
                    time_point %in% comb) %>% 
      ungroup() %>% 
      get_group_comparisons(.,link_var = "record_id", long_var = "time_point",cat_vector = "group_bmi",num_vector = "richness",type = "longitudinal", comps = comb,graph_coords = T)
    
    obj %>% 
      pluck("group_bmi","richness","test") %>% 
      rstatix::adjust_pvalue( method = "BH", p.col = "p") %>% 
      full_join(obj$group_bmi[["richness"]]$stats, by = "group")%>% 
      dplyr::mutate(.y. = "richness") %>% 
      dplyr::select(group, group1, group2,num_var = .y.,y.position, xmin, xmax, contains("median"), contains("iqr"), "p.adj") 
  })%>% 
