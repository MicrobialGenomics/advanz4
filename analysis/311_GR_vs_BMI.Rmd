...
```{r alluvial_plot, warning = F, message = F, echo = show}
all_bmis %>%
  ...
  geom_flow(stat = "alluvium",  na.rm = F, width = 0.4, color = "black", alpha = .8) +
  geom_stratum() +
  geom_text(stat = "alluvium", aes(label = record_id, color = group_bmi), size = 2.7) +
  scale_color_manual(values= c("black","white","white", "darkgrey")) +
  theme_minimal() +
...
corr_BL <-
  ...
  # geom_label(data = test_cors_OA, aes(label = paste("spearman rho =",cor, "p =", p), x = min(delta_bmi_df$gr0), y = Inf), vjust=1, hjust = 0.1)+
...
corr_BL <-
  ...
  # geom_label(data = test_cors_group, aes(label = paste("spearman rho =",cor, "p =", p), x = min(delta_bmi_df$gr0), y = Inf, color = group), vjust=2, hjust = 0.1)+
  labs(title = "BMI vs GR",subtitle = "per time", x = "Gene richness at baseline", y = "BMI at baseline") +
  theme_light() +
  scale_color_manual(values = c("#AAAAAA", "#AAAAAA", "#000000"), limits = c("bDRV", "DTG", "overall")) +
  scale_fill_manual(values = c("#AAAAAA","#AAAAAA","#000000"), limits = c("bDRV", "DTG", "overall")) +
  scale_linetype_manual(values = c(2,3,1), limits = c("bDRV", "DTG", "overall")) 
...
gr0_bmi_cattest <- 
  delta_bmi_df %>% 
  dplyr::group_by(time_point, group) %>% 
...
delta_bmi_df %>%
  dplyr::filter(!SampleID == "ADZ4_27") %>%
  # unique() %>%
  ggplot(aes(x = group_bmi, y = gr0, fill = group)) +
  facet_wrap(~ time_point ) +
  geom_boxplot() +
  stat_compare_means(method = "wilcox.test", label = "..p.signif..", hide.ns = F)+
  labs(title = "BMI vs GR",subtitle = "per time", x = "BMI at baseline", y = "Gene richness at baseline") +
  stat_pvalue_manual(data = gr0_bmi_cattest, hide.ns = T) +
  theme_light() +
  scale_color_manual(values = c("#AAAAAA", "#000000"), limits = c("bDRV", "DTG")) +
  scale_fill_manual(values = c("#AAAAAA","#000000"), limits = c("bDRV", "DTG")) 
...
```{r delta_cors_BL, warning=FALSE, message=F, fig.height=600,echo = show, fig.width=800}
delta_bmi_df <-
    igc_df %>%
...
deltas_gr0_abs <-
  ...
  geom_text(data = test_gr0_abs, 
             ...
             inherit.aes = F, vjust=1, hjust = 0.1)+
...
Finally, we can move on to stratify the cohort on BMI classes
...
cat_test <-
  ...
  get_group_comparisons(
    ...
    link_var = "record_id",
    long_var = "time_point",
    cat_vector = "group_bmi",
    num_vector = "richness",
    type = "categorical",
    comps = unique(delta_bmi_df$group_bmi),
    graph_coords = T
  )

objtest <- cat_test %>% pluck("group_bmi", "richness", "test")
objstats <- cat_test %>% pluck("group_bmi", "richness", "stats") %>%
...
long_test <- 
combn(unique(delta_bmi_df$time_point), 2, simplify = F) %>% 
  purrr::set_names() %>% 
  purrr::map_dfr(function(comb){
    ...
    delta_bmi_df %>% 
      dplyr::filter(!is.na(richness),
                    time_point %in% comb) %>% 
      ungroup() %>% 
      get_group_comparisons(.,link_var = "record_id", long_var = "group_bmi",cat_vector = "time_point",num_vector = "richness",type = "categorical", comps = comb,graph_coords = T)
    
    obj %>% 
      pluck("time_point","richness","test") %>% 
      rstatix::adjust_pvalue( method = "BH", p.col = "p") %>% 
      full_join(obj$time_point[["richness"]]$stats, by = "cat_var")%>% 
      dplyr::mutate(.y. = "richness") %>% 
      dplyr::select(long_var, group1, group2,num_var = .y.,y.position, xmin, xmax, contains("median"), contains("iqr"), "p.adj") 
  })%>% 
