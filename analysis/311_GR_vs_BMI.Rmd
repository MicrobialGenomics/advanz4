...
```{r basal_dist_df, warning = F, message = F}
peaks_bl <- igc_df %>% 
...
```{r kmeans_split, warning=FALSE, message=FALSE, eval = T, echo = T}
...
igc_df %>% 
  ...
  dplyr::select(record_id = link_var,long_var, contains("delta"), bmi0, gr0,richness ,cat_var) 
...
peaks_rel <- 
  ...
  purrr::map_dfr(function(treat) {
    ...
    delta_bmi_df %>%
      dplyr::filter(long_var == 24, !is.na(delta_BMI_rel)) %>% 
      dplyr::filter(cat_var == treat) %>% 
      dplyr::pull(delta_BMI_rel) %>%
      density() %>%
      peak_finder(goal = "min") %>%
