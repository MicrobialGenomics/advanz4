...
myClinicalData<-here::here("Metadata","MetadataManagement","Export.xlsx") %>%
  readxl::read_excel() %>%
...
vars=c("CD4 value","CD8 value", "4+38+DR+","8+38+DR+", "PCR", "IL-6", "TNF-a", "sCD14","HIV Viral load [central]")
names(vars) <- c("CD4","CD8", "CD4_CD38_DR","CD8_CD38_DR", "CRP", "IL6", "TNFa", "sCD14", "viral_load")
  ...
  dplyr::mutate(
    method = recode(
     ...
     `8+38+DR+` = "CD8_CD38_DR",
      ...
      `TNF-a` = "TNFa",
      sCD14 = "sCD14",
     `HIV Viral load [central]` = "viral_load"
     ),
