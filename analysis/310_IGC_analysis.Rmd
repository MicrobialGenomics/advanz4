...
There is a general increase even without stratifying for any confounding variable.
...
group_plots
ggplot2::ggsave(plot = group_plots, path =here::here("output","figures"), filename = "GR_group_boxplots.svg", device = "svg", width = 20,height = 15, units = "cm", dpi = 300)
...
```{r differences_table, warning = F, message = F}
igc_df <- TableList$group


obj <-
  igc_df %>%
  dplyr::filter(!is.na(richness), !is.na(cat_var)) %>%
  get_group_comparisons(
    ...
    num_vector = "richness",
    type = "categorical",
    comps = unique(igc_df$cat_var)
  ) 

objtest <- obj %>% pluck("cat_var", "richness", "test") %>% 
  rstatix::adjust_pvalue(p.col = "p", method = "BH")
objstats <- obj %>% pluck("cat_var", "richness", "stats") %>%
...
obj %>%
  ...
  full_join(obj$cat_var$richness$stats, by = "cat_var") %>%
  # dplyr::mutate(.y. = nv) %>%
