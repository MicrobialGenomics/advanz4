...
mymre@metadata@categorical_vals <-  
  here::here(myPath, "Metadata","CategoricalVariables.txt") %>%
  read.table(., sep="\t") %>%
  setNames(c("CategoricalVariable","PaletteName")) %>%
  as_tibble()

mymre@metadata@metadata_df <-  
  here::here(myPath, "Metadata","2022_03_25_clean_metadata_LIMS.csv") %>%
  read.csv(.) %>%
  as_tibble()
...
samples <- metadata %>% filter(record_id %in% onesample) %>% pull(SampleID)

metar::filter_samples(mre = mymre,sample_ids = samples)
