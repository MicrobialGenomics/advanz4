...
```{r load_u_data_cars, eval=T, include=F, eval=F}
here<-here::here()
# setwd(myPath)
mymre<-here::here(here, "data", "mre.rds") %>%
  readRDS()
...
```{r get_GeneRichness_Data, echo=FALSE, message=F, warning=F, eval=F}
...
``` {r Plot_Rarefaction, echo=F, message=F, warning=F, eval=F}
myRarefaction<-mymre@diversity@igc@rarefaction$categorical$Group$rarefaction +
  scale_color_manual(values = brewer.pal(n=2, name =  "Set1"), labels = c("ABC+3TC+DGT","ABC+3TC+DRV+RTV")) + ### Keep labels consistent throughout. It is DTG, not DGT. Better to use DTG and DRV/r as labels (but DRVr in metadata)
...
```{r taxa_functions, echo = F, warning=F, message = F, eval=F}
...
```{r Taxa_Boxplots_Phyl, echo = F, warning=F, message=F, eval=F}
...
```{r Taxa_Boxplots_Gen, echo = F, warning=F, message=F, eval=F}
