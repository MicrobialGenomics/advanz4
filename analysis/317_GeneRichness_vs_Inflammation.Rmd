...
## Clinical evolution of inflammation markers

We'll perform here the basic exploratory analysis of the evolution in inflammation, so we have an idea of what to expect when correlating it to GR.

```{r plot_inflammation, warning=F, message=FALSE}
...
num_var %>% 
  ...
  purrr::map(function(nv){
    ...
    unit <- 
      ifelse(nv %in% c("CD4_CD38_DR", "CD8_CD38_DR"), "Abundance",
                metadata %>% 
                  dplyr::pull(!!sym(paste("units", nv, sep ="_"))) %>% 
               na.omit() %>% 
               unique())
      name <- nv %>% 
        str_replace_all(., "\\_", "\\+ ") %>% 
    ...
    metadata %>% 
      dplyr::select(SampleID, record_id, time_point, group, !!sym(nv)) %>% 
      dplyr::mutate(group = "overall") %>% 
      rbind(metadata[,c("SampleID", "record_id", "time_point", "group",nv) ]) %>% 
      dplyr::filter(!is.na(!!sym(nv))) %>% 
      dplyr::mutate(time_point = factor(time_point, levels = unique(time_point)),
                    !!sym(nv) := as.numeric(!!sym(nv))) %>% 
      get_comp_boxplots(., num_var = nv,long_var = "time_point",link_var = "record_id", cat_var = "group", include.pvals = "categorical") %>% 
      purrr::pluck("plot") +
      scale_color_manual(values = c("#AAAAAA", "#777777", "#000000")) +
      scale_fill_manual(values = c("#AAAAAA", "#777777", "#000000")) +
      # geom_jitter(aes(x = long_var, color = cat_var, shape = cat_var),position = position_jitterdodge(jitter.width = .1)) +
      labs(color = "group",
           shape = "group",
           y = unit,
           title = name) +
...
plot_grid <- 
ggarrange(plotlist = plot_list, ncol = 2, nrow = 4, common.legend = T) %>% 
  ggpubr::annotate_figure(bottom = text_grob("Week", size = 12, face = "bold"))


ggsave(plot_grid, device = "svg", filename = here::here("output", "figures", "inflammation.svg"), height = 20, width = 15, units = "cm")
...
## Inflammation increases vs baseline GR




```{r gr0_deltas, warning = F, message=F}
...
igc_df <- 
  ...
  left_join(metadata %>% 
    ...
    by = c("SampleID")) %>% 
  # dplyr::select(-ADV) %>% 
  dplyr::rename(richness = GeneNumber) %>% 
...
delta_df <- 
  igc_df %>% 
  dplyr::mutate(gr0 = richness[long_var == 0],
                across( num_var, ~ 100*(.x - .x[long_var == 0])/.x[long_var == 0])) 
...
num_var[!num_var == "ADV"] %>% 
  ...
  purrr::map(function(nv){
    ...
    test <- delta_df %>% 
      ...
      rstatix::cor_test(vars = c("gr0", nv), method = "spearman")
    
    name <- nv %>% 
      str_replace_all(., "\\_", "\\+ ") %>% 
      str_replace_all(string = ., pattern = "DR", replacement="DR+")
    
    delta_df %>% 
      dplyr::filter(!long_var == 0) %>% 
      ggplot(aes(x = !!sym(nv), y = gr0)) +
      geom_point(aes(color = cat_var)) +
      stat_smooth(method = "glm", color = "black") +
      labs(x = "increase (%)", 
           y = "Gene richness at baseline",
           title = name,
           subtitle = paste("rho =", test$cor, "p =", test$p, sep = " "),
           color = "group") +
      theme_minimal() + 
      theme(axis.title.x = element_blank(),
...
plt_cor_grid <- 
ggarrange(plotlist = pltcorlist, common.legend = T, ncol = 2, nrow = 4) %>% 
  ggpubr::annotate_figure(left = text_grob(label = "Gene richness at baseline", size = 14, face = "bold", rot = 90),
                          bottom = text_grob(label = "Increase from baseline (%)", size = 14, face = "bold"))

ggsave(plt_cor_grid, device = "svg", filename = here::here("output", "figures", "inf_gr_corr.svg"), width = 18, height = 25, units = "cm")
