...
```{r venn_long_group, warning=F, message=F}
...
stat_list_long$CD4diff_48 %>%
      dplyr::select(species, group2, level) %>%
      dplyr::mutate(row=row_number()) %>% 
      tidyr::pivot_wider(., values_from = "species", names_from = c("level", "group2")) %>% 
      dplyr::select(-row) %>%
  ggVennDiagram::ggVennDiagram(., show_intersect = T,title="CD4 difference 48")


    
stat_list_long$CD4diff_96 %>%
      dplyr::select(species, group2, level) %>%
      dplyr::mutate(row=row_number()) %>% 
      tidyr::pivot_wider(., values_from = "species", names_from = c("level", "group2")) %>% 
      dplyr::select(-row) %>%
