...
metadata <- 
  metadata %>% 
  dplyr::mutate(group = str_replace(group, "DRV/r", "bDRV"))
...
grplot <- boxplots$group$plot +
  labs(y = "Gene count", title = "\u03b1-diversity") +
...
plt <- 
ggarrange(grplot,B.a, B.l, O.s, common.legend = T, ncol = 1, align = "hv", widths = c(1.2,1))%>% 
  annotate_figure(., bottom = text_grob(label = "Week", face = "bold", size = 14))


metadata %>% 
  group_by(record_id, group) %>% 
    filter(time_point %in% c(0,96),
         n() == 2) %>% 
  arrange(time_point) %>% 
  select(record_id, group, CD4) %>% 
  summarise(delta = diff(CD4)) %>% 
  group_by(group, .drop = T) %>% 
  summarise(median(delta))
  
metadata %>% 
  # dplyr::select(record_id, group) %>% 
  # group_by(group) %>% 
  # filter(time_point %in% c(0)) %>% 
  # wilcox_test(CD4 ~ group)carl
    group_by(group) %>%

  summarise(med = median(CD4_nadir),
            lower_ci = quantile(CD4_nadir, .25),
            upper_ci = quantile(CD4_nadir, .75))

ggsave(plot = plt, filename = "output/poster_boxplots.svg", width = 14, height = 28, units = "cm")
```
