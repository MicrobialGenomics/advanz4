...
```{r venn_long_group, warning=F, message=F, eval=F}
...
group_diff <- names(comps) %>% # DA species common between 48 and 96 for each group
  set_names() %>% 
  map(~ {
    Venn_Group %>% 
      ggVennDiagram::Venn(.) %>% 
      ggVennDiagram::discern_overlap(., slice = comps[[.x]])
  }) 
...
```{r plot_DA_species, warning=F, message=F, eval=F}
  ...
  purrr::map(function(cv) {
    ...
    map(function(sp){
      ...
      stat_params <- 
        ...
        rename(cat_var = level) %>% 
        mutate(xmax=case_when(group2 == 48 ~ 2, group2 == 96 ~ 3),
               y.position = 1.1*max)
      
      otu_df %>%
        dplyr::select(SampleID,!!sym(sp)) %>%
        dplyr::left_join(metadata[c("SampleID",link_var, long_var, cv)]) %>%
        dplyr::filter(time_point %in% c(0,48,96)) %>%
        dplyr::select(subject.id = !!sym(link_var), long_var = long_var, cat_var = !!sym(cv), sp = !!sym(sp)) %>%
        dplyr::arrange(subject.id) %>%
        dplyr::mutate(long_var = as.factor(long_var)) %>%
        ggplot(., aes(x = long_var, y =sp, fill = cat_var)) +
        facet_wrap(facets = "cat_var") +
        geom_violin(width=1.5, trim = T) +
        geom_boxplot(width = 0.1, fill="white", ) +
        geom_line(aes(group = subject.id), color="grey") +
        scale_fill_brewer(palette = "Set1") +
        ggpubr::stat_pvalue_manual(stat_params, label = "p", xmin = "xmin", xmax="xmax", y ="y.position") + 
        theme_bw() +
        labs(x="week", x=sp, fill =cv)
