...
author: "Marc Noguera, Carlos Blázquez"
date: "5/3/2022"
...
```{r read_data, warning=FALSE, message=FALSE, cache=T}
...
```{r construct_nmds, warning = F, message = F, cache = T}
mymre <- metaphlan_nmds(mymre, tax_level = "Species", top_n = 50, save_files = F)

group_nmds <- get_taxa(mymre, type="metaphlan", result_slot = "nmds") %>% 
  purrr::pluck("Species", "top_50","categorical","group")
```



```{r nmds_ADV, warning=F, message = F, cache= T, fig.width= 10, fig.height= 8, eval=F}
...
c("group","ADV_p") %>% 
  ...
  group_nmds$data %>% 
  dplyr::select(SampleID, NMDS1, NMDS2) %>% 
  dplyr::left_join(igc, by="SampleID") %>% 
  dplyr::inner_join(metadata_f[,c("SampleID","ADV", "record_id", cv)], by="SampleID") %>% 
  # get_lmm_effects(data = ., cat_vector = c("group","ADV_p"), num_vector = "ADV", long_var = comp, link_var = "record_id") %>% purrr::pluck("group","ADV","plot") +
  # scale_x_continuous(n.breaks = 10) +
  ggplot(., aes(x=))
  labs(x = "NMDS1", title = comp)
...
group_nmds$data %>% 
  ...
  left_join(igc, by="SampleID") %>% 
  left_join(metadata_f[,c("SampleID","ADV")], by="SampleID") %>% 
  dplyr::filter(!ADV == max(.[,"ADV"], na.rm = T)) %>% 
  ggplot(aes(x=ADV, y = NMDS2, color = group)) +
  geom_point() 
