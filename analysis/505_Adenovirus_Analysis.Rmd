...
myLMMs <- get_lmm_effects(data =metadata, cat_vector = c("group"), num_vector = c("ADV"), long_var = long_var) 
...
We will test for presence/absence and try to test for differences among groups. We'll test for proportions of patients positive per ADV per group and time.
...
metadata %>% 
  ...
  ggplot(., aes(x=long_var, y = incidence, fill = num_var)) +
  facet_wrap(~cat_var) +
  geom_histogram(stat="identity", position = "stack") +
  theme_minimal() +
  scale_fill_brewer(palette = "Set1") +
  labs(x = "week",y="incidence (%)", fill = "ADV")
