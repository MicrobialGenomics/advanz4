...
```{r plot_basic_NMDS, warning = F, include = F, eval = F, message=F, warning=F}
mymre <- metar::metaphlan_nmds(mre = mymre, tax_level = "Species", save_files = F)

# mymre@taxa@metaphlan@nmds$Species$top_50$categorical %>%
  # discard(., names(.) == "record_id")

mymre@taxa@metaphlan@nmds
metar::get_diversity(mymre, type = "metaphlan", res_slot = "nmds")
