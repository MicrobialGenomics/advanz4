...
mymre@taxa@metaphlan@phyloseq@sam_data <- metadata %>%
  ...
  mutate(SampleID = rownames(.), .before=1) %>% 
...
basal_mre@metadata@categorical_vals <- get_cat(mymre) %>% 
  filter(!CategoricalVariable %in% c("ethnic_group"))


phyloseq::sample_data(basal_mre@taxa@metaphlan@phyloseq) <- basal_meta
phyloseq::sample_data(basal_mre@taxa@metaphlan@phyloseq_sec) <- basal_meta
...
ggpubr::ggarrange(plotlist = nmdscor, ncol=2)
