...
basal_samples <- metadata %>%
  filter(time_point == 0) %>% 
  pull(SampleID) %>% 
  as.character()

basal_mre <- filter_samples(mymre, basal_samples)
# basal_mre@metadata@categorical_vals <-remove_cat(basal_mre, "CD4diff_96")


basal_mre@metadata@categorical_vals <- get_cat(mymre) %>% 
  filter(!CategoricalVariable %in% c("ethnic_group","CD8after_48","CD4diff_96", "CD4diff_48"))
...
get_meta(basal_mre) %>% pull(CD8diff_96)
...
```{r adonis _table, echo=F, warning=F, message = F}
...
# Numerical variables vs unsupervised clustering


```{r numerical_vs_cluster}
