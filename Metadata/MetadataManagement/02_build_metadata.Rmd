...
myClinicalData<-here::here("Metadata","MetadataManagement","Clinical_Table.csv") %>%
  read.csv() %>%
  select(names(variables_selected)) %>%
  mutate(PatCode = gsub("-","",PatCode)) %>%
  mutate(Group=ifelse(Treat == "ABC+3TC+DGT", 1, 2)) %>%
  setNames(nm=c(variables_selected,"Group"))
...
MetadataLab<-here::here("Metadata", "MetadataManagement", "LabData.xlsx") %>%
  ...
  left_join(myMergedMetadata, ., by=c("record_id","TimePoint"), all=T) 
...
MetadataLabDem<- here::here("Metadata","MetadataManagement","Vital.xlsx") %>%
  ...
  left_join(MetadataLab,. ,by=c("record_id","TimePoint"), all.x = T) %>%
  relocate(record_id, TimePoint, Group, names(vars),Weight, Height, BMI, SBP, DBP)
...
metadata_final <- c(48) %>%
  ...
  map_dfr( ~ {
    ...
    metadata_final %>% 
      ...
      mutate(CD4diff = case_when(
        CD4[TimePoint == .x] - CD4[TimePoint == 0] >= 50 ~ ">50",
        CD4[TimePoint == .x] - CD4[TimePoint == 0] < 50 ~ "<50"),
      CD8diff = case_when(
        CD8[TimePoint == .x] - CD8[TimePoint == 0] >= 50 ~ ">50",
        CD8[TimePoint == .x] - CD8[TimePoint == 0] < 50 ~ "<50"),
      CD4after = case_when(
        CD4[TimePoint == .x]  < 200 ~ "low",
        CD4[TimePoint == .x]  >= 200 & CD4[TimePoint == .x] <= 500 ~ "mid",
        CD4[TimePoint == .x]  > 500 ~ "high"),
      CD8after = case_when(
        CD8[TimePoint == .x]  < 200 ~ "low",
        CD8[TimePoint == .x]  >= 200 & CD8[TimePoint == .x] <= 500 ~ "mid",
        CD8[TimePoint == .x]  > 500 ~ "high")) %>%
  ...
  relocate(SampleID, record_id, TimePoint, Group, names(vars), contains("diff"), contains("after"))
...
myCompTable<-compareGroups(Group ~ Gender + Age + EthnicGroup, myClinicalData, simplify = F) %>% createTable()
...
sampleData<-here::here("Metadata","MetadataManagement","Informacio_LIMS.xlsx") %>%
  ...
  mutate(record_id = sapply(strsplit(gsub("^4*","",record_id), "-"), "[[",1),
         ...
         TimePoint = ifelse(TimePoint == "Basal", 0,
                            gsub("^w*", "", TimePoint))) %>% 
  unique() 
